...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #AKUYATEST2\r");
            Send(hSocket, "WHO #AKUYATEST2\r");
        } else {
            // randomly join #AKUYA00-#AKUYA05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #AKUYA%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #AKUYA%02d\r", channel_number).c_str());
        }
