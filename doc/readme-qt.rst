...
Alternatively, install Qt Creator and open the `AKUYA-qt.pro` file.

An executable named `AKUYA-qt` will be built.
...
A warning for people using the *static binary* version of AKUYA on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of AKUYA is linked against libdb5.3.
...
installed causes AKUYA-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
